...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/07 17:26:29 by hcanon            #+#    #+#             */
/*   Updated: 2020/03/05 16:43:21 by hcanon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_List(void)
{
	...
	std::cout << "\n---push_back\n\n";
	{
		...
		typename ft::ReverseIterator<typename ListType::Itlist> rit = lst.rbegin();
		typename ft::ReverseIterator<typename ListType::Itlist> rite = lst.rend();
		i = 1;
		std::cout << "(reverse) [5, 42, 3]:" << '\n';
		while (rit != rite)
			std::cout << i++ << " -> " << *rit++ << '\n';

		typename ListType::Itlist	it = lst.begin();
		typename ListType::Itlist	ite = lst.end();
	...
	std::cout << "\n---push_front\n\n";
	{
		...
		typename ListType::Itlist it;
		typename ListType::Itlist ite;
	...
	std::cout << "\n---erase\n\n";
	{
		...
		typename ListType::Itlist it = lst.erase(--lst.end());
	...
	std::cout << "\n---unique (binary pred)\n\n";
	{
		...
		display_container("[5]: {lesser_than_rhs}:", lst);
			assert(lst.size() == 1);

		lst.unique(&lesser_than_rhs<T>);
		display_container("[5]: {lesser_than_rhs: no change}:", lst);
			assert(lst.size() == 1);
	...
	std::cout << "\n---merge\n\n";
	{
		...
		display_container("[5, 12, 28, 42, 43, 44, 45, 5] {lst after merge}:", lst);
			assert(lst.size() == 8);
